...
tem = xr.open_dataset(path + '/DATA/DEW_cun.nc')
model = load_model(path + '/MODELOS/modelo_dew.h5')
model.load_weights(path + '/MODELOS/modelo_dew_pesos.h5')
...
if os.path.exists(path + '/SALIDAS/' + 'prediccion_dew.nc' ):
    os.remove(path + '/SALIDAS/' + 'prediccion_dew.nc')
...
ncout = Dataset(path + '/SALIDAS/' + 'prediccion_dew.nc','w','NETCDF4')
...
myvar = ncout.createVariable('d2m','float32',('time','lat','lon'));myvar.setncattr('units','°C');myvar[:] = dataout;
ncout.close();