...
tem = xr.open_dataset(path + '/DATA/U_cun.nc')
model = load_model(path + '/MODELOS/modelo_u10.h5')
model.load_weights(path + '/MODELOS/modelo_u10_pesos.h5')
...
if os.path.exists(path + '/SALIDAS/' + 'prediccion_u10.nc' ):
    os.remove(path + '/SALIDAS/' + 'prediccion_u10.nc')
...
ncout = Dataset(path + '/SALIDAS/' + 'prediccion_u10.nc','w','NETCDF4')
...
myvar = ncout.createVariable('u10','float32',('time','lat','lon'));myvar.setncattr('units','m/s');myvar[:] = dataout;
ncout.close();