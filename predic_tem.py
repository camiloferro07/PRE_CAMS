...
tem = xr.open_dataset(path + '/DATA/TEM_cun.nc')
model = load_model(path + '/MODELOS/modelo_tem.h5')
model.load_weights(path + '/MODELOS/modelo_tem_pesos.h5')
...
if os.path.exists(path + '/SALIDAS/' + 'prediccion_tem.nc' ):
    os.remove(path + '/SALIDAS/' + 'prediccion_tem.nc')
...
ncout = Dataset(path + '/SALIDAS/' + 'prediccion_tem.nc','w','NETCDF4')
...
myvar = ncout.createVariable('t2m','float32',('time','lat','lon'));myvar.setncattr('units','°C');myvar[:] = dataout;
ncout.close();