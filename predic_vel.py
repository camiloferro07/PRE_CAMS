...
tem = xr.open_dataset(path + '/DATA/VEL_cun.nc')
model = load_model(path + '/MODELOS/modelo_vel.h5')
model.load_weights(path + '/MODELOS/modelo_vel_pesos.h5')
...
if os.path.exists(path + '/SALIDAS/' + 'prediccion_vel.nc' ):
    os.remove(path + '/SALIDAS/' + 'prediccion_vel.nc')
...
ncout = Dataset(path + '/SALIDAS/' + 'prediccion_vel.nc','w','NETCDF4')
...
myvar = ncout.createVariable('vel','float32',('time','lat','lon'));myvar.setncattr('units','m/s');myvar[:] = dataout;
ncout.close();